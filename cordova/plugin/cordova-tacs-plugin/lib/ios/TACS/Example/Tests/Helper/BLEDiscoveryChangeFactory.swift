...
class BLEDiscoveryChangeFactory {
    ...
    static func discoveryStartedChange(with sorcID: SorcID, date: Date = Date()) -> DiscoveryChange {
        let sorcInfo = SorcInfo(sorcID: sorcID, discoveryDate: date, rssi: 1)
        let sorcInfoByID = [sorcID: sorcInfo]
        let sorcInfos = SorcInfos(sorcInfoByID)
        let state = SecureAccessBLE.DiscoveryChange.State(discoveredSorcs: sorcInfos, discoveryIsEnabled: true, requestedSorc: sorcID)
        let action = SecureAccessBLE.DiscoveryChange.Action.discoveryStarted(sorcID: sorcID)
        let discoveryChange = SecureAccessBLE.DiscoveryChange(state: state, action: action)
        return discoveryChange
    }
}
