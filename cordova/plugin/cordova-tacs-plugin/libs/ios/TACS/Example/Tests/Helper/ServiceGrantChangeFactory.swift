...
class ServiceGrantChangeFactory {
    ...
    static func acceptedTelematicsRequestChange() -> ServiceGrantChange {
        let state = ServiceGrantChange.State(requestingServiceGrantIDs: [TelematicsManager.telematicsServiceGrantID])
    ...
    static func acceptedLocationRequestChange() -> ServiceGrantChange {
        let state = ServiceGrantChange.State(requestingServiceGrantIDs: [LocationManager.locationServiceGrantID])
        let action = ServiceGrantChange.Action.requestServiceGrant(id: LocationManager.locationServiceGrantID, accepted: true)
        return ServiceGrantChange(state: state, action: action)
    }
}
