...
class TelematicsManagerTests: QuickSpec {
    ...
    override func spec() {
        ...
        var locationManager: LocationManagerDefaultMock!

        beforeEach {
            sorcManager = SorcManagerMock()
            locationManager = LocationManagerDefaultMock()
            sut = TelematicsManager(sorcManager: sorcManager, locationManager: locationManager, queue: DispatchQueue(label: "com.queue.ble"))
                ...
                context("data was requested") {
                    ...
                    context("service grant id is telematics") {
                        context("accepteed") {
                            ...
                            it("notifies change") {
                                let expectedAction = TelematicsDataChange.Action.requestingData(types: [.odometer], accepted: true)
                        ...
                        context("not accepted") {
                            beforeEach {
                                let state = ServiceGrantChange.State(requestingServiceGrantIDs: [])
                                let action = ServiceGrantChange.Action.requestServiceGrant(id: TelematicsManager.telematicsServiceGrantID,
                                                                                           accepted: false)
                                serviceGrantChange = ServiceGrantChange(state: state, action: action)
                                consumeResult = sut.consume(change: serviceGrantChange)
                            }
                            ...
                            it("notifies change") {
                                let expectedAction = TelematicsDataChange.Action.requestingData(types: [], accepted: false)
                                expect(telematicsDataChange) == TelematicsDataChange(state: [], action: expectedAction)
                            }
                        }
                    }
            ...
            describe("requestTelematicsData") {
                context("connected and already requesting") {
                    ...
                    it("notifies updated requesting change") {
                        ...
                        if case let .requestingData(types, _)? = telematicsDataChange?.action {
