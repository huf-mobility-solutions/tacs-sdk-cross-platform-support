...
class TACSManagerTests: QuickSpec {
    ...
    override func spec() {
        ...
        beforeEach {
            ...
            let locationManager = LocationManagerDefaultMock()
        ...
        describe("use grant from keyring") {
            ...
            context("with crafted keyring") {
                it("succeeds") {
                    let keyRing = TACSKeyRingFactory.validDefaultKeyRing()
                    let grantID = keyRing.tacsLeaseTokenTable.first!.vehicleAccessGrantId
                    let craftedKeyRing = TACSKeyRing.craftKeyring(leaseTokenId: keyRing.tacsSorcBlobTable.first!.blob.sorcId,
                                                                  leaseId: keyRing.tacsLeaseTokenTable.first!.leaseToken.leaseId,
                                                                  sorcId: keyRing.tacsSorcBlobTable.first!.blob.sorcId,
                                                                  sorcAccessKey: keyRing.tacsSorcBlobTable.first!.externalVehicleRef,
                                                                  blob: keyRing.tacsSorcBlobTable.first!.blob.blob,
                                                                  blobMessageCounter: keyRing.tacsSorcBlobTable.first!.blob.blobMessageCounter,
                                                                  externalVehicleRef: keyRing.tacsSorcBlobTable.first!.externalVehicleRef,
                                                                  vehicleAccessGrantId: grantID,
                                                                  keyholderId: keyRing.tacsSorcBlobTable.first!.keyholderId)
                    let result = sut.useAccessGrant(with: grantID, from: craftedKeyRing)
                    expect(result) == true
                }
            }
        }
        ...
        describe("scan with timout") {
            let keyRing: TACSKeyRing = {
                TACSKeyRingFactory.validDefaultKeyRing()
            }()
            var grantID: String = ""
            let vehicleRef = "4321"
            let date = Date()
            ...
            beforeEach {
                grantID = keyRing.tacsLeaseTokenTable.first!.vehicleAccessGrantId
            }
            context("active vehicle set") {
                it("starts discovery with timeout") {
                    _ = sut.useAccessGrant(with: grantID, from: keyRing)
                    sut.scanInternalWithTimeout(0.1)
                    expect(sorcManager.timeoutForDiscovery) == TimeInterval(0.1)
                    expect(sorcManager.didReceiveStartDiscoveryForSpecificSorc) == 1
                }
            }
            context("discovery change with timeout") {
                it("notifies discovery start") {
                    let bleDiscoveryChange = BLEDiscoveryChangeFactory.discoveryStartedChange(with: keyRing.sorcID(for: vehicleRef)!, date: date)

                    let expectedChange = TACSDiscoveryChangeFactory.discoveryStartedChange(with: vehicleRef, date: date)
                    _ = sut.useAccessGrant(with: grantID, from: keyRing)
                    sut.scanInternalWithTimeout(0.1)
                    sorcManager.discoveryChangeSubject.onNext(bleDiscoveryChange)
